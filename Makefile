...
install: ./driver 
	...
	install -Dm644 src-tauri/icons/512x512.png "${DESTDIR}usr/share/icons/hicolor/512x512/apps/io.attackshark.driver.png"
.PHONY: uninstall
uninstall:
	rm "${DESTDIR}usr/bin/attack-shark" "${DESTDIR}etc/attack-shark/config.toml" "${DESTDIR}usr/lib/udev/rules.d/99-attack-shark-r1.rules" "${DESTDIR}usr/lib/udev/rules.d/99-attack-shark-x11.rules" "${DESTDIR}usr/share/applications/io.attackshark.driver.desktop" "${DESTDIR}usr/share/icons/hicolor/512x512/apps/io.attackshark.driver.png"
